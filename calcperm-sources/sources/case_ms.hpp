...
#ifndef unit__case_ms
#define unit__case_ms
...
public:
  ...
  t_bool init();
  void finalize();
  t_int find(const string& s, t_int not_found);
  };
...
#define CASE(s) \
      ...
      if (_l_ == 0) { \
        static tr_string_entry _e_ = {0, (s), LBL(0)}; \
        _e_.next = _m_.map[0]; \
        _m_.map[0] = &_e_; \
        } \
      else \
...
#define CASE_MULTI(s) \
      ...
      { \
        enum {_x_ = LBL(0)}; \
        if (_l_ == 0) { \
          static tr_string_entry _e_ = {0, (s), (t_int)(_x_)}; \
          _e_.next = _m_.map[0]; \
          _m_.map[0] = &_e_; \
          }
