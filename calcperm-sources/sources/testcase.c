...
// Test program
// gcc
// see case_*.c for additional info

// Options for case_map.c:
...
int main(UNUSED(int argc), const char* argv[]) {
  ...
  printf("Testing switch on string in C...\n");
  printf("input: %s\n",s);
  ...
  LOOP(3)
    SWITCH(s)
      CASE("abc") printf("abc found\n");
      CASE("def") printf("def found\n");
      CASE("ghi") printf("ghi found\n");
      CASE("jkl") printf("jkl found, jump to mno\n");  goto mno;
      CASE("mno")
      mno:
        printf("mno found\n");
      CASE("aaa") printf("aaa found\n");
      CASE("aab") printf("aab found\n");
      CASE("aba") printf("aba found\n");
      DEFAULT printf("nothing found\n");
      END
    END
  ...
  BEGIN
    ...
    WHILE a2[idx].i != 0 DO
      ...
      IF i != a2[idx].i DO
        ok = false;
        printf("Internal error (a2[%i].i=%i)!\n", idx, a2[idx].i);
        END
      ++idx;
      END
    END
  ...
  BEGIN
    ...
    IF i != 3 DO
      ok = false;
      printf("Internal error SWITCH+break!\n");
      END
    END
  ...
  LOOP(100000)
    ...
    WHILE a1[idx].i != 0 DO
      ...
      IF i != a1[idx].i DO
        ok = false;
        printf("Internal error (a1[%i].i=%i => %i)!\n",
          idx, a1[idx].i, i);
        END
      ++idx;
      END
